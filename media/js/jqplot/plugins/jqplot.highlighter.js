...
(function($) {
    ...
    $.jqplot.Highlighter = function(options) {
        ...
        this.markerRenderer = new $.jqplot.MarkerRendererBorder({shadow:false});
        // prop: showMarker
    ...
    function draw(plot, neighbor) {
        ...
        mr.style = smr.style;
        mr.lineWidth = smr.lineWidth;
        mr.size = smr.size + hl.sizeAdjust;
        mr.color = smr.color;
        mr.borderColor = smr.borderColor;
        mr.init();
    ...
    function showTooltip(plot, series, neighbor) {
        ...
        var arr = neighbor.data[2].slice();
        arr.unshift(hl.formatString);
        var str = $.jqplot.sprintf.apply($.jqplot.sprintf, arr);
        ...
        for(k=plot.seriesStack.length-1; k>=0; k--) {
                ...
                if (s.renderer.constructor == $.jqplot.LineRenderer && s.show)
                {
                    ...
                    for (var j=0; j<s.gridData.length; j++) {
                        p = s.gridData[j];
                        if(p[0] != null && p[1] != null && Math.abs(p[0] - gridpos.x) < min_dist){
                            min_dist = Math.abs(p[0] - gridpos.x);
                            neighbor = {seriesIndex: i, pointIndex:j, gridData:p, data:s.data[j]};
                        }
                    }
                }
        }
